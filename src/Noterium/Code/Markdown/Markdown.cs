	...
	public class Markdown : DependencyObject
	{
	   ...
	   DependencyProperty.Register("HighlightedTextStyle", typeof(Style), typeof(Markdown), new PropertyMetadata(null));
		...
		private Block BlockQuoteEvaluator(Match match)
		{
			...
			string quote = match.Groups[0].Value;
			quote = quote.TrimStart('>').Replace("\n>", "\n");
			var result = new Run(quote.Trim());

			Paragraph pg = new Paragraph(result);
			if (QuoteStyle != null)
			{
				pg.Style = QuoteStyle;
			}
		...
		private Block CodeBlocksEvaluator(Match match)
		{
			...
			string codeBlock = match.Groups[0].Value;
		...
		private Block TableNoAlignEvaluator(Match match)
		{
			...
			string headString = match.Groups[1].Value;
			string rowsString = match.Groups[2].Value;
		...
		private Block TableEvaluator(Match match)
		{
			...
			string headString = match.Groups[1].Value;
			string textAlign = match.Groups[2].Value;
			string rowsString = match.Groups[3].Value;
		...
		{
			string[] headColumns = headString.Split(new[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
			...
			{
				string[] textAlignColumns = textAlign.Split(new[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
			...
			for (int i = 0; i < headColumns.Length; i++)
			{
				string rowColumn = headColumns[i];
				Paragraph p = new Paragraph(new Run(rowColumn.Trim()));
			...
			{
				string trimmed = rowString.Trim();
				string[] rowColumns = trimmed.Split(new[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
				...
				{
					string rowColumn = rowColumns[i];

					string text = rowColumn.Trim();
		...
		{
			TextAlignment[] alignments = new TextAlignment[textAlignColumns.Length];
			for (int i = 0; i < textAlignColumns.Length; i++)
			{
				string textAlignColumn = textAlignColumns[i];
		...
		private IEnumerable<Block> FormParagraphs(string text)
		{
			...
			string[] grafs = RegularExpressions.NewlinesMultiple.Split(RegularExpressions.NewlinesLeadingTrailing.Replace(text, ""));

			return grafs.Select(g =>
			{
				Paragraph p = Create<Paragraph, Inline>(RunSpanGamut(g));
		...
		private Dictionary<string, LinkReference> GetAnchorSources(string text)
		{
			...
			Dictionary<string, LinkReference> result = new Dictionary<string, LinkReference>();
			...
			foreach (Match m in matches)
			{
				...
				string url = m.Groups["link"].Value;
		...
		private Inline AnchorInlineEvaluator(Match match, bool highlight)
		{
			...
			if (_anchorSources.ContainsKey(url))
			{
				reference = _anchorSources[url];
			}
		...
		private Inline MathEvaluator(Match match)
		{
			...
			string mathExpression = match.Groups[0].Value;
		...
		private Block ImageInlineEvaluator(Match match)
		{
			...
			NoteFile nf = Note.Files.FirstOrDefault(n => n.FileName == url);
			...
			if (nf != null && File.Exists(nf.FullName))
			{
				...
				if (!string.IsNullOrWhiteSpace(options))
				{
					...
					Regex reg = new Regex(@"=(?:(?<width>\d{1,})x?(?<height>\d{1,})?)?;?(?<float>\w{1,})?", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace | RegexOptions.Singleline | RegexOptions.Compiled);

					Match m = reg.Match(options);
					...
					if (floatGroup != null && floatGroup.Success)
					{
						floatDirection = floatGroup.Value;
					}
				}
				...
				{
					BlockUIContainer imgContainer = new BlockUIContainer { Child = image };

					Floater fl = new Floater(imgContainer);
					if (image.Width > -1)
					...
					else if (floatDirection.Equals("right"))
					{
						fl.HorizontalAlignment = HorizontalAlignment.Right;
					}
		...
		private Block ListEvaluator(Match match)
		{
			...
			string listType = Regex.IsMatch(match.Groups[3].Value, SharedSettings.MarkerUl) ? "ul" : "ol";
		...
		private Block TodoListEvaluator(Match match)
		{
			...
			string[] list = content.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries);

			StackPanel sp = new StackPanel();
			BlockUIContainer ctnr = new BlockUIContainer(sp);

			foreach (string s in list)
			{
				string text = s.Substring(s.IndexOf("] ", StringComparison.Ordinal) + 2).Trim();
		...
		private Inline StrikeEvaluator(Match match, int contentGroup, bool highlight)
		{
			...
			var content = match.Groups[contentGroup].Value;
			Span result = Create<Span, Inline>(RunSpanGamut(content, highlight));
			...
			for (int i = 0; i < text.Length; i++)
			{
				switch (text[i])
				{
					...
					case '\r':
						...
						break;
					case '\t':
						int width = (SharedSettings.TabWidth - line.Length % SharedSettings.TabWidth);
