...
namespace Noterium.Code.Markdown
{
    public class XamlFormatter : DependencyObject
    {
            ...
            while (block != null)
            {
                ...
                switch (block.Tag)
                {
                    ...
                    case BlockTag.BlockQuote:
                        var blockquoteParagraph = new Section();
                        if (trackPositions)
                    ...
                    case BlockTag.ListItem:
                        ...
                        {
                            var ctnr = (BlockUIContainer) blockParent;
                            var sp = (StackPanel) ctnr.Child;

                            visitChildren = false;
                            var spcb = new TextBlock();
                            spcb.Style = TodoTextStyle;
                            var checkBoxInline = block.FirstChild?.InlineContent;
                    ...
                    case BlockTag.FencedCode:
                        var codeblockParagraph = new Paragraph();
        ...
        {
            var withinLink = false;
            var stackWithinLink = false;
            ...
            while (inline != null)
            {
                ...
                switch (inline.Tag)
                {
                    ...
                    case InlineTag.RawHtml:
                        ...
                        break;
                    case InlineTag.Code:
                        var codeSpan = new Span(new Run(inline.LiteralContent));
                ...
                switch (inline.Tag)
                {
                    ...
                    case InlineTag.Link:
                        ...
                        else
                        {
                            ...
                            var url = inline.TargetUrl;
                            if (uriResolver != null) url = uriResolver(inline.TargetUrl);
                    ...
                    case InlineTag.Strikethrough:
                        var strikethroughSpan = new Span();
        ...
        {
            var nf = CurrentNote.Files.FirstOrDefault(n => n.FileName == inline.TargetUrl);
            var result = parent as Paragraph;
            if (result == null)
            {
        ...
        {
            if ((settings.AdditionalFeatures & CommonMarkAdditionalFeatures.GithubStyleTables) == 0) throw new CommonMarkException("Table encountered in AST, but GithubStyleTables are not enabled");
            ...
            while (curHeaderCell != null)
            {
                ...
                if (alignment != TableHeaderAlignment.None)
                    switch (alignment)
                    {
                        case TableHeaderAlignment.Center:
                            cell.TextAlignment = TextAlignment.Center;
                            break;
                        case TableHeaderAlignment.Left:
                            cell.TextAlignment = TextAlignment.Left;
                            break;
                        case TableHeaderAlignment.Right:
        ...
        {
            get => (ICommand) GetValue(CheckBoxCheckedCommandProperty);
        ...
        {
            get => (Note) GetValue(CurrentNoteProperty);
